...
let zoomedLocation = null; // New variable to store zoomed-in location
...
zoomInButton.onAdd = function () {
 ...
 button.onclick = function () {
   if (userLocation) {
     ...
     const center = bounds.getCenter(); // Get the center of the current view
     const gridSize = 0.144; // 32km in latitude/longitude approximately
     const newBounds = L.latLngBounds(
       [center.lat - gridSize / 2, center.lng - gridSize / 2],
       [center.lat + gridSize / 2, center.lng + gridSize / 2]
     );
...
function zoomToArea(location) {
  const cascadeRangeCoords = [40.5, -122.0]; // Coordinates for the Cascade Range area in California
  const bounds = L.latLngBounds(
    [cascadeRangeCoords[0] - 0.144 / 2, cascadeRangeCoords[1] - 0.144 / 2],
    [cascadeRangeCoords[0] + 0.144 / 2, cascadeRangeCoords[1] + 0.144 / 2]
  );
